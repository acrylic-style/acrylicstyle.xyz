...
const fetchRepos = async () => {
  ...
  while (true) {
    if (index === -1) return;
    await fetch(`https://next.acrylicstyle.xyz/api/repos.php?page=${++index}`).then(res => res.json()).then(data => {
      if (data.length === 0) {
        index = -1
...
const refreshRepoList = async () => {
  ...
  repoList.forEach(repo => {
    ...
    && (!options.hidePrivate || !repo.private)
    && (!options.hideOrg || repo.owner.login === 'acrylic-style')
    && (!options.hidePublic || repo.private)) {
      repoClone.push(repo)
    }
  })
